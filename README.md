...
3. **Run the attacker.** Right-click **BruteForceClient → Set as Startup
   ...
   1. **Show the web page** — the program acts like a web browser and just
      fetches a page (a quick "is it connected?" check).
   2. **Do a login** — you type a username and password and it sends that one
      attempt. Use the demo account to see success, then a wrong password to see
      failure.
   3. **Run my own code** — the empty `StudentTask()` method where you write the
      solution.
4. **Do the task.** Follow the student worksheet in `docs/`.
...
1. Set bob's password in `TargetWebsite/Program.cs` to the number you want the
   ...
   left as is.
2. Fill in the bracketed placeholders in the student worksheet (your repo link,
...
4. Keep `docs/Teacher Notes & Answer Guide.docx` for yourself — it has model
